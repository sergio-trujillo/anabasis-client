...
export function MockPowerDayPage() {
  ...
  if (!examQuery.data || examQuery.data.rounds.length === 0) {
    return (
      <AppLayout>
  ...
  return <Runner key={examQuery.data.examId} exam={examQuery.data} />;
}

type ExamData = NonNullable<ReturnType<typeof trpc.mock.buildPowerDay.useQuery>["data"]>;
type Round = ExamData["rounds"][number];
...
function RoundBody({ round }: { round: Round }) {
  ...
  if (ex.type === "code") {
    return <CodeExercise exercise={ex} />;
  }
  if (ex.type === "open-prompt") {
    return <OpenPromptExercise exercise={ex} />;
  }
  if (ex.type === "interviewer-chat") {
    return <InterviewerChatExercise exercise={ex} />;
  }
  return (
    <Card>
      <CardContent className="p-6 text-muted-foreground text-sm">
        Unsupported round type: {ex.type}
      </CardContent>
    </Card>
  );
}
